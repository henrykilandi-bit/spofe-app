...
async function verifyAssociations() {
  try {
    ...
    // Compagnie ↔ JournalEntry
    if (models.Compagnie && models.JournalEntry) {
      const hasCompanyToJE = models.Compagnie.associations?.entries;
      const hasJEToCompany = models.JournalEntry.associations?.company;
      
      if (hasCompanyToJE && hasJEToCompany) {
        logSuccess('Compagnie ↔ JournalEntry: BIDIRECTIONNELLE');
      } else {
        logError('Compagnie ↔ JournalEntry: INCOMPLÈTE');
        errors.push('Compagnie.hasMany(JournalEntry) OU JournalEntry.belongsTo(Compagnie) manquant');
      }
    }

    // Compagnie ↔ ChartOfAccount
    if (models.Compagnie && models.ChartOfAccount) {
      const hasCompanyToCOA = models.Compagnie.associations?.chartsOfAccounts;
      const hasCOAToCompany = models.ChartOfAccount.associations?.company;
      
      if (hasCompanyToCOA && hasCOAToCompany) {
        logSuccess('Compagnie ↔ ChartOfAccount: BIDIRECTIONNELLE');
      } else {
        logError('Compagnie ↔ ChartOfAccount: INCOMPLÈTE');
        errors.push('Compagnie.hasMany(ChartOfAccount) OU ChartOfAccount.belongsTo(Compagnie) manquant');
      }
    }
