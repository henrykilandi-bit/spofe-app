...
exports.CostStructureRepository = void 0;
const economic_project_aggregate_1 = require("../domain/economic-project.aggregate");
const value_objects_1 = require("../domain/value-objects");
class CostStructureRepository {
    constructor(db) {
        this.db = db;
    }
    /**
     ...
     */
    async saveCostLine(projectId, version, costLine) {
        const query = `
    ...
    `;
        ...
        return new economic_project_aggregate_1.EconomicProject(row.project_id, row.tenant_id, row.name, row.type, row.status, versions, row.created_by, row.created_at, row.validated_at, row.validated_by, row.rejected_at, row.rejected_by, row.rejection_reason);
    }
    /**
    ...
    `;
        ...
        for (const row of result.rows) {
            ...
            const assumptions = row.price_target
                ? new value_objects_1.EconomicAssumptions(new value_objects_1.Money(row.price_target), new value_objects_1.Quantity(row.expected_volume), new value_objects_1.Quantity(row.capacity_max), new value_objects_1.EconomicScenarios(row.scenario_pessimistic, row.scenario_realistic, row.scenario_optimistic))
                : undefined;
            const simulationMetrics = row.unit_cost
                ? new value_objects_1.SimulationMetrics(new value_objects_1.Money(row.unit_cost), new value_objects_1.Money(row.total_cost), row.gross_margin, row.net_margin, row.margin_at_70, row.viable_at_70)
                : undefined;
            versions.push({
                version: row.version,
                status: row.status,
     ...
     */
    async loadCostLines(projectId, version) {
        const query = `
    ...
    `;
        ...
        return result.rows.map((row) => ({
            ...
            label: row.label,
            amount: new value_objects_1.Money(row.amount, row.currency),
...
exports.CostStructureRepository = CostStructureRepository;
//# sourceMappingURL=cost-structure.repository.js.map