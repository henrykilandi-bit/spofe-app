import { ChartOfAccount, Company, JournalEntryLine } from '../models/index.js';
 ...
 * @param {Object} req - Requête Express (query: company_id, includeInactive)
 * @param {Object} res - Réponse Express
 */
export const getAllAccounts = async (req, res) => {
  try {
    let { company_id, includeInactive = 'false' } = req.query;

    // ✅ VALIDATION company_id OBLIGATOIRE
    if (!company_id) {
    ...
    // if (!req.user.companies.includes(company_id)) {
    ...
    const accounts = await ChartOfAccount.findAll({
      ...
      include: [
        ...
        },
        {
          model: ChartOfAccount,
          as: 'subAccounts',
          attributes: ['id', 'accountNumber', 'accountName', 'is_active'],
          required: false
        }
      ],
...
export const createAccount = async (req, res) => {
  try {
    ...
    if (parentAccountId) {
      ...
      if (parentAccount.company_id !== company_id) {
...
export const updateAccount = async (req, res) => {
  try {
    ...
    // Mettre à jour les champs autorisés (on ne permet pas de changer le numéro de compte ou company_id)
 ...
 * @param {Object} req - Requête Express (query: company_id, search, accountType)
 * @param {Object} res - Réponse Express
 */
export const searchAccounts = async (req, res) => {
  try {
    const { company_id, search, accountType } = req.query;
...
export const seedOHADAAccounts = async (req, res) => {
  try {
    ...
    const accountIds = await ChartOfAccount.findAll(
      { where: { company_id: company.id }, attributes: ['id'] }
    );
    ...
    await ChartOfAccount.destroy({ where: { company_id: company.id } });
