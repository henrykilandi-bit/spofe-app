...
// Setup relationships
Company.hasMany(ChartOfAccount, { foreignKey: 'company_id' });
ChartOfAccount.belongsTo(Company, { foreignKey: 'company_id' });
...
Company.hasMany(AccountBalance, { foreignKey: 'company_id' });
AccountBalance.belongsTo(Company, { foreignKey: 'company_id' });
...
async function testUserModel() {
  ...
  try {
    ...
    // UPDATE
    await foundUser.update({ is_active: false });
    const updatedUser = await User.findByPk(user.id);
    logTest('User UPDATE', updatedUser.is_active === false, 'User updated successfully');
...
async function testCompanyModel() {
  ...
  try {
    ...
    // UPDATE
    await foundCompany.update({ is_active: false });
    const updatedCompany = await Company.findByPk(company.id);
    logTest('Company UPDATE', updatedCompany.is_active === false, 'Company updated successfully');
...
async function testChartOfAccountModel() {
  ...
  try {
    ...
    const accountByNumber = await ChartOfAccount.findOne({
      where: { company_id: company.id, accountNumber: '101' }
    });
    logTest('ChartOfAccount FIND_BY_NUMBER', accountByNumber?.id === childAccount.id, 'Account found by number');

    // UPDATE
    await foundChild.update({ is_active: false });
    const updatedAccount = await ChartOfAccount.findByPk(childAccount.id);
    logTest('ChartOfAccount UPDATE', updatedAccount.is_active === false, 'Account updated successfully');
...
async function testJournalEntryModel() {
  ...
  try {
    ...
    const entryByNumber = await JournalEntry.findOne({
      where: { company_id: company.id, entryNumber: 'JE/2024/001' }
    });
...
async function testForeignKeyConstraints() {
  ...
  try {
    ...
    logTest('FK_COMPANY_ACCOUNT', !!account?.id && account.company_id === company.id, 'Account FK to Company works');
