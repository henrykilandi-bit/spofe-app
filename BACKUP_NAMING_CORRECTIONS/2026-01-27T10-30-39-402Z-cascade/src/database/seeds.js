...
export const seedCompanies = async (Compagnie) => {
  try {
    const companies = [
      {
    ...
    for (const companyData of companies) {
      const existing = await Compagnie.findOne({
      ...
      if (!existing) {
        ...
        console.log(`✅ Compagnie created: ${companyData.name}`);
      }
    }
...
export const seedChartOfAccounts = async (ChartOfAccount, Compagnie) => {
  try {
    ...
    const company = await Compagnie.findOne();
...
export const runAllSeeds = async (sequelize) => {
  try {
    const { User, Compagnie, ChartOfAccount } = sequelize.models;

    console.log('🌱 Starting database seeds...\n');

    await seedAdminUser(User);
    await seedTestUsers(User);
    await seedCompanies(Compagnie);
    await seedChartOfAccounts(ChartOfAccount, Compagnie);
