...
const useSuperUser = () => {
   ...
   */
  const approveUser = useCallback(async (userId, notes = '') => {
    try {
      setLoading(true);
      setError(null);
   ...
   */
  const rejectUser = useCallback(async (userId, reason, notes = '') => {
    try {
      setLoading(true);
      setError(null);
   ...
   */
  const requestChanges = useCallback(async (userId, requirements) => {
    try {
      setLoading(true);
      setError(null);
      
      const response = await axiosInstance.post(
        `/admin/users/${userId}/request-changes`,
        { requirements }
      );
