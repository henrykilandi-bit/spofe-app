...
exports.PostgresBudgetRepository = void 0;
const value_objects_1 = require("../domain/value-objects");
class PostgresBudgetRepository {
    constructor(pool) {
        this.pool = pool;
    }
    async save(budget, client) {
    ...
    async findById(id, tenantId, client) {
        const conn = client || this.pool;
        const query = `
    ...
    async findByPeriod(period, tenantId, client) {
        const conn = client || this.pool;
        const query = `
      SELECT * FROM budget_objectif
      WHERE tenant_id = $1
    ...
    mapRowToBudget(row) {
        return {
            ...
            period: new value_objects_1.Period(new Date(row.period_start), new Date(row.period_end), row.period_granularity),
            status: row.status,
