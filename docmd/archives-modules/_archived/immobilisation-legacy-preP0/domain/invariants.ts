...
import { Money, UsefulLife, Percentage, Period, DateRange, AssetStatus } from './value-objects';
...
export const AssetInvariants = {
  ...
  validateAcquisitionCostPositive(cost: Money): void {
    if (cost.amount <= 0) {
      throw new InvariantViolation(
        InvariantCodes.ASSET_ACQUISITION_COST_POSITIVE,
  ...
  validateResidualValueNonNegative(residualValue: Money): void {
    if (residualValue.amount < 0) {
      throw new InvariantViolation(
        InvariantCodes.ASSET_RESIDUAL_VALUE_NON_NEGATIVE,
        'Residual value must be non-negative',
        { amount: residualValue.amount }
      );
    }
  },
  ...
  validateAcquisitionDatePast(acquisitionDate: Date): void {
    ...
    if (acquisitionDate > now) {
      throw new InvariantViolation(
        InvariantCodes.ASSET_ACQUISITION_DATE_PAST,
  ...
  validateNotDisposed(status: AssetStatus): void {
    if (status === 'DISPOSED' || status === 'DECOMMISSIONED') {
      throw new InvariantViolation(
        InvariantCodes.ASSET_DISPOSED_IMMUTABLE,
        'Cannot modify a disposed or decommissioned asset',
        { status }
      );
    }
  },
  ...
  validateInServiceForDepreciation(status: AssetStatus): void {
    if (status !== 'IN_SERVICE') {
      throw new InvariantViolation(
        InvariantCodes.ASSET_NO_DEPRECIATION_IF_NOT_IN_SERVICE,
        'Depreciation can only be recorded for assets in service',
        { status }
      );
    }
  },
  ...
  validateResidualValueWithinCost(residualValue: Money, acquisitionCost: Money): void {
    if (residualValue.isGreaterThan(acquisitionCost)) {
      throw new InvariantViolation(
        InvariantCodes.ASSET_RESIDUAL_VALUE_LESS_THAN_COST,
        'Residual value cannot exceed acquisition cost',
        { residualValue: residualValue.amount, acquisitionCost: acquisitionCost.amount }
      );
    }
  },
};
...
export const DepreciationInvariants = {
  ...
  validateDepreciationAmountNonNegative(amount: Money): void {
    if (amount.amount < 0) {
      throw new InvariantViolation(
        InvariantCodes.DEPRECIATION_AMOUNT_NON_NEGATIVE,
  ...
  validateAccumulatedWithinCost(accumulated: Money, acquisitionCost: Money): void {
    if (accumulated.isGreaterThan(acquisitionCost)) {
      throw new InvariantViolation(
        InvariantCodes.DEPRECIATION_ACCUMULATED_WITHIN_COST,
        'Accumulated depreciation cannot exceed acquisition cost',
        { accumulated: accumulated.amount, acquisitionCost: acquisitionCost.amount }
      );
    }
  },
  ...
  validateNetBookValueFormula(
    ...
    if (Math.abs(netBookValue.amount - expected.amount) > 0.01) {
      throw new InvariantViolation(
        InvariantCodes.DEPRECIATION_NBV_EQUALS_FORMULA,
        'Net book value must equal acquisition cost minus accumulated depreciation',
        { 
  ...
  validateNetBookValueAboveResidual(netBookValue: Money, residualValue: Money): void {
    if (netBookValue.amount < residualValue.amount - 0.01) {
      throw new InvariantViolation(
        InvariantCodes.DEPRECIATION_NBV_ABOVE_RESIDUAL,
        'Net book value cannot fall below residual value',
        { netBookValue: netBookValue.amount, residualValue: residualValue.amount }
      );
    }
  },
};
...
export const AllocationInvariants = {
  ...
  validateAllocationSum100(percentages: Percentage[]): void {
    if (!Percentage.isComplete(percentages)) {
      throw new InvariantViolation(
        InvariantCodes.ALLOCATION_SUM_100,
        'Total allocation percentages must equal 100%',
        { sum: Percentage.sum(percentages) }
      );
    }
  },
  ...
  validateNoOverlap(newRange: DateRange, existingRanges: DateRange[]): void {
    for (const existing of existingRanges) {
      if (newRange.overlaps(existing)) {
        throw new InvariantViolation(
          InvariantCodes.ALLOCATION_NO_OVERLAP,
          'Allocation periods cannot overlap for the same asset and target',
          { 
  ...
  validateAssetNotDisposed(status: AssetStatus): void {
    if (status === 'DISPOSED' || status === 'DECOMMISSIONED') {
      throw new InvariantViolation(
        InvariantCodes.ALLOCATION_FORBIDDEN_IF_DISPOSED,
...
export const MaintenanceInvariants = {
  ...
  validateCostNonNegative(cost: Money): void {
    if (cost.amount < 0) {
      throw new InvariantViolation(
        InvariantCodes.MAINTENANCE_COST_NON_NEGATIVE,
  ...
  validateDatePast(date: Date): void {
    ...
    if (date > now) {
      throw new InvariantViolation(
        InvariantCodes.MAINTENANCE_DATE_PAST,
  ...
  validateAssetInService(status: AssetStatus): void {
    if (status !== 'IN_SERVICE') {
      throw new InvariantViolation(
        InvariantCodes.MAINTENANCE_ASSET_IN_SERVICE,
        'Maintenance can only be recorded for assets in service',
        { status }
      );
    }
  },
...
export const DisposalInvariants = {
  ...
  validateDisposalDateAfterAcquisition(disposalDate: Date, acquisitionDate: Date): void {
    if (disposalDate < acquisitionDate) {
      throw new InvariantViolation(
        InvariantCodes.DISPOSAL_DATE_AFTER_ACQUISITION,
        'Disposal date must be on or after acquisition date',
        { 
  ...
  validateAssetInService(status: AssetStatus): void {
    if (status !== 'IN_SERVICE') {
      throw new InvariantViolation(
        InvariantCodes.DISPOSAL_ASSET_IN_SERVICE,
        'Only assets in service can be disposed',
        { status }
      );
    }
  },
  ...
  validateGainLossFormula(gainOrLoss: Money, disposalValue: Money, netBookValue: Money): void {
    ...
    if (Math.abs(gainOrLoss.amount - expected.amount) > 0.01) {
      throw new InvariantViolation(
        InvariantCodes.DISPOSAL_GAIN_LOSS_FORMULA,
        'Gain/Loss must equal disposal value minus net book value',
        { 
...
export const SecurityInvariants = {
  ...
  validateTenantIdPresent(tenantId: string | undefined | null): void {
    if (!tenantId || tenantId.trim().length === 0) {
      throw new InvariantViolation(
        InvariantCodes.TENANT_ID_REQUIRED,
  ...
  validateSameTenant(expectedTenantId: string, actualTenantId: string): void {
    if (expectedTenantId !== actualTenantId) {
      throw new InvariantViolation(
        InvariantCodes.NO_CROSS_TENANT,
        'Cross-tenant operation is forbidden',
