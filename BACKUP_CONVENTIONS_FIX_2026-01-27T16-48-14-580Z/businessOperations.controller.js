...
import { businessOperationDto, businessOperationDtoArray } from '../dto/index.js';
...
class BusinessOperationController {
    ...
    try {
      ...
      await BusinessOperationAudit.logAction(
        ...
        req.user.id,
        {
          ...
          newValues: businessOperationDto(operation),
          ipAddress: req.ip,
  ...
  async list(req, res) {
    try {
      ...
      const { count, rows } = await BusinessOperation.findAndCountAll({
        ...
        order: [['operationDate', 'DESC'], ['created_at', 'DESC']]
      });
  ...
  async update(req, res) {
    ...
    try {
      ...
      const oldValues = businessOperationDto(operation);
  ...
  async validate(req, res) {
    ...
    try {
      ...
      // Obtenir company_id depuis l'opération (via fiscal_year)
    ...
    try {
      ...
      await BusinessOperationAudit.logAction(
        ...
        req.user.id,
        {
          ...
          oldValues: businessOperationDto(operation),
          ipAddress: req.ip,
          userAgent: req.get('user-agent')
        }
      );
