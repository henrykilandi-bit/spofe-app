...
interface ParsedArgs {
  ...
  configPath: string;
  rules?: string[];
  format: 'console' | 'json' | 'junit' | 'github';
  verbose: boolean;
...
class AGA {
  ...
  private checkLayerSeparation(filePath: string, content: string): void {
  ...
  private checkCommandPurity(filePath: string, content: string): void {
    const relativePath = path.relative(this.args.scanPath, filePath);
  ...
  private checkNoDirectMutation(filePath: string, content: string): void {
    const relativePath = path.relative(this.args.scanPath, filePath);
  ...
  private checkGuardianUsage(filePath: string, content: string): void {
    const relativePath = path.relative(this.args.scanPath, filePath);
  ...
  private checkProcessNesting(filePath: string, content: string): void {
    const relativePath = path.relative(this.args.scanPath, filePath);
  ...
  private generateReport(): AGAReport {
  ...
  private reportConsole(report: AGAReport): void {
    ...
    if (report.violations.length === 0) {
      ...
      const groupedByRule: Record<string, Violation[]> = {};
      report.violations.forEach((v) => {
        if (!groupedByRule[v.rule]) groupedByRule[v.rule] = [];
  ...
  private reportJSON(report: AGAReport): void {
    console.log(JSON.stringify(report, null, 2));
  }

  private reportGitHub(report: AGAReport): void {
    report.violations.forEach((v) => {
