...
export const refreshTokenEndpoint = async (req, res, next) => {
  try {
    ...
    if (isBlacklisted) {
      logger.logSecurity('Attempt to use blacklisted refresh token', { user_id: decoded.id });
    ...
    if (!user || !user.is_active) {
      logger.logSecurity('Refresh token for inactive/deleted user', { user_id: decoded.id });
    ...
    logger.logInfo('Token refreshed successfully', { user_id: user.id });
...
export const logout = async (req, res, next) => {
  try {
    ...
    try {
      ...
      logger.logInfo('User logged out successfully', { user_id: decoded.id });
...
export const requestPasswordReset = async (req, res, next) => {
  try {
    ...
    logger.logInfo('Password reset requested', { user_id: user.id, email });
...
export const resetPassword = async (req, res, next) => {
  try {
    ...
    if (redisToken !== token) {
      logger.logSecurity('Reset token revoked', { user_id: user.id });
    ...
    logger.logInfo('Password reset completed', { user_id: user.id });
...
export const getCurrentUser = async (req, res, next) => {
  try {
    ...
    if (!user) {
      logger.logSecurity('Attempt to access deleted user', { user_id: req.user.id });
      return unauthorized(res, 'User not found');
    }

    if (!user.is_active) {
      logger.logSecurity('Attempt to access inactive user', { user_id: req.user.id });
...
export const verifyUserActive = async (req, res, next) => {
  try {
    ...
    if (!user || !user.is_active) {
      logger.logSecurity('Access attempt by inactive user', { user_id: req.user.id });
      return forbidden(res, 'User account is inactive');
    }
