# Direct Guardian HTTP Mapping Tests - Simplified
...
try {
    $response = Invoke-WebRequest -Uri "$ServerUrl/health" -Method GET -ContentType "application/json" -TimeoutSec 5 -ErrorAction Stop
    if ($response.StatusCode -eq 200) {
        $content = $response.Content | ConvertFrom-Json
        if ($content.status) {
            Write-Host "  ✓ PASS - Status 200, Key 'status' found" -ForegroundColor Green
            $TestsPassed++
        } else {
            Write-Host "  ✗ FAIL - Status 200 OK, but key 'status' missing" -ForegroundColor Red
            $TestsFailed++
        }
    } else {
...
try {
    ...
    $response = Invoke-WebRequest -Uri "$ServerUrl/api/v1/aggregates" -Method POST -Body $body -ContentType "application/json" -TimeoutSec 5 -ErrorAction Stop
    ...
    if ($_.Exception.Response) {
        ...
        if ($status -eq 403) {
            try {
                ...
                if ($errorContent.violation) {
                    Write-Host "  ✓ PASS - Status 403, Key 'violation' found" -ForegroundColor Green
                    $TestsPassed++
                } else {
                    Write-Host "  ✗ FAIL - Status 403 OK, but key 'violation' missing" -ForegroundColor Red
                    $TestsFailed++
                }
            } catch {
...
try {
    ...
    if ($response.StatusCode -eq 201) {
        $content = $response.Content | ConvertFrom-Json
        if ($content.status) {
            Write-Host "  ✓ PASS - Status 201, Key 'status' found" -ForegroundColor Green
            $TestsPassed++
        } else {
            Write-Host "  ✗ FAIL - Status 201 OK, but key 'status' missing" -ForegroundColor Red
            $TestsFailed++
        }
    } else {
...
try {
    $response = Invoke-WebRequest -Uri "$ServerUrl/api/v1/nonexistent-resource" -Method GET -ContentType "application/json" -TimeoutSec 5 -ErrorAction Stop
    ...
    if ($_.Exception.Response) {
        ...
        if ($status -eq 404) {
            ...
            Write-Host "  ✗ FAIL - Expected 404, got $status" -ForegroundColor Red
            $TestsFailed++
        }
    } else {
        Write-Host "  ✗ FAIL - Connection error: $($_.Exception.Message)" -ForegroundColor Red
...
try {
    $response = Invoke-WebRequest -Uri "$ServerUrl/api/profile" -Method GET -ContentType "application/json" -TimeoutSec 5 -ErrorAction Stop
...
if ($TestsFailed -eq 0) {
    ...
    Write-Host "  ✓ Guardian rejection for USER role (403 with G4-03)" -ForegroundColor Green
