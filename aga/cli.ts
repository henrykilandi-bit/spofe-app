...
function main() {
  ...
  const enrichedReport = ReportAggregator.build(
    analysisReport.files.length,
    analysisReport.files.flatMap(f => f.violations)
  );
  ...
  if (!enrichedReport.isCompliant) {
    const hasBlockers = enrichedReport.bySeverity.some(
      s => s.severity === 'error' || s.severity === 'blocker'
    );
