...
export const getDashboardSummary = asyncHandler(async (req, res) => {
    ...
    if (!company_id) {
      throw new AppError("ID de l'entreprise requis", 400, { field: 'company_id' });
    }
    ...
    const activityRows = await sequelize.query(
      ...
      {
        replacements: { company_id, startDate: activityStartDate },
    ...
    logger.logInfo('Dashboard summary retrieved successfully', { user_id: req.user?.id, company_id });
...
export const getDashboardActivity = asyncHandler(async (req, res) => {
    ...
    const operations = await sequelize.query(
      `SELECT DATE(bo.created_at) AS date, COUNT(*) AS count
       ...
       GROUP BY DATE(bo.created_at)`,
      {
        replacements: { company_id, startDate: start, endDate: end },
        type: QueryTypes.SELECT,
      }
    );
    ...
    logger.logInfo('Dashboard activity retrieved successfully', { user_id: req.user?.id, company_id, period });
