...
const { User, Company, ChartOfAccount, JournalEntry, JournalEntryLine, AccountBalance } = db;
...
async function seed() {
  try {
    ...
    let company = await Company.findOne({ where: { registrationNumber: 'CI-001-2024' } });
    ...
    const accounts = await ChartOfAccount.findAll({ where: { company_id: company.id } });
