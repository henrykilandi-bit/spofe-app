...
export const createIndicator = async (req, res, next) => {
  try {
    ...
    logger.logInfo(`Indicateur créé: ${indicator.id} pour objectif ${objectiveId} par ${userId}`);

    return success(res, { indicator: indicator.toJSON() }, 201, 'Indicateur créé');
  } catch (err) {
...
export const recordKpiValue = async (req, res, next) => {
  try {
    ...
    logger.logInfo(`Valeur enregistrée pour indicateur ${indicatorId}: ${value} par ${userId}`);
...
export const updateIndicator = async (req, res, next) => {
  try {
    ...
    logger.logInfo(`Indicateur ${indicatorId} mis à jour par ${userId}`);
...
export const deleteIndicator = async (req, res, next) => {
  try {
    ...
    logger.logInfo(`Indicateur ${indicatorId} supprimé (soft-delete) par ${userId}`);
...
export const bulkRecordValues = async (req, res, next) => {
  try {
    ...
    logger.logInfo(`${results.filter(r => r.success).length} valeurs enregistrées en bulk par ${userId}`);
