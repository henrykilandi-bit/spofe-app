...
import { ImmobilisationEvent, AllocationCreatedEvent, AllocationEndedEvent, createBaseEvent } from './events';
...
export class AssetAllocation {
  private constructor(
    ...
    public readonly target: AllocationTarget,
  ...
  static create(params: {
    ...
    actorId: string;
  }): { aggregate: AssetAllocation; events: ImmobilisationEvent[] } {
    ...
    const aggregate = new AssetAllocation(
      params.allocationId,
      params.assetId,
      params.tenantId,
      target,
  ...
  static fromState(props: AssetAllocationProps): AssetAllocation {
    return new AssetAllocation(
      ...
      props.assetId,
      props.tenantId,
      new AllocationTarget(props.targetType, props.targetId),
      props.percentage,
      props.effectiveFrom,
      props.effectiveTo,
      props.createdAt,
  ...
  endAllocation(params: {
    ...
    const event: AllocationEndedEvent = {
...
export class AllocationValidator {
  ...
  static validateTotalAllocation(
    ...
    date: Date
  ): { isValid: boolean; total: number; missing: number } {
   ...
   */
  static getActiveAllocationsForPeriod(
    allocations: AssetAllocation[],
    periodStart: Date,
    periodEnd: Date
  ): AssetAllocation[] {
