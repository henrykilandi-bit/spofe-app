...
// 1.1 Mettre à jour les scripts référençant /backend/
const scriptFiles = findFiles('/backend/', ['.sh', '.ps1', '.js', '.json', '.md', '.yml', '.yaml']);
console.log(`📄 Fichiers trouvés avec "/backend/": ${scriptFiles.length}`);
...
scriptFiles.forEach(file => {
  ...
  try {
    ...
    // Remplacer les références /backend/ par /cascade/
...
console.log(`🔧 Corrections /backend/ → /cascade/: ${backendCorrections} fichiers`);
