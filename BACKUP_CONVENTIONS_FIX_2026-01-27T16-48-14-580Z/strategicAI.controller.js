...
export const generateSmartGoals = async (req, res, next) => {
  try {
    ...
    logger.logInfo(`${generatedGoals.length} smart goals générés pour ${compagnie.nom} par ${user_id}`);
...
export const generateStrategicReport = async (req, res, next) => {
  try {
    ...
    logger.logInfo(`Rapport stratégique généré pour ${compagnie.nom} par ${user_id}`);
...
export const generateSectorBenchmarkReport = async (req, res, next) => {
  try {
    ...
    logger.logInfo(`Rapport benchmark généré pour ${compagnie.nom} vs ${secteur} par ${user_id}`);
...
export const batchAiAnalysis = async (req, res, next) => {
  try {
    ...
    logger.logInfo(`Analyse batch ${results.filter(r => r.success).length}/${objectiveIds.length} complétée par ${user_id}`);
