...
class OptimizedJournalController {
  ...
  async getEntries(req, res, next) {
    try {
      ...
      if (!compagnieId) {
        return badRequest(res, 'Company ID required', 400);
      }
  ...
  async getEntryWithLines(req, res, next) {
    try {
      ...
      if (!id || !compagnieId) {
        return badRequest(res, 'ID and Company ID required');
      }
      ...
      if (!entry) {
        logger.warn('Journal entry not found', { entryId: id, userId });
  ...
  async getReport(req, res, next) {
    try {
      ...
      if (!periode || !compagnieId) {
        return badRequest(res, 'Periode and Company ID required');
      }
  ...
  async getAccountBalances(req, res, next) {
    try {
      ...
      if (!compagnieId || !startDate || !endDate) {
        return badRequest(res, 'Company ID, start date and end date required');
      }
  ...
  async getAccountJournal(req, res, next) {
    try {
      ...
      if (!accountId || !periode || !compagnieId) {
        return badRequest(res, 'Account ID, periode and Company ID required');
      }
