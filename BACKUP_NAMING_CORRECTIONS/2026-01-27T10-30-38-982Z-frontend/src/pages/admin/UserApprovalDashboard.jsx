...
const UserApprovalDashboard = () => {
  ...
  const approveUser = async (userId, bulk = false) => {
    try {
      const token = localStorage.getItem('token');
      const apiUrl = process.env.REACT_APP_API_URL || 'http://localhost:3001';
      ...
      const response = await axios.post(
        `${apiUrl}/api/admin/users/${userId}/approve`,
        { note },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const rejectUser = async (userId, reason, note = '') => {
    try {
      const token = localStorage.getItem('token');
      const apiUrl = process.env.REACT_APP_API_URL || 'http://localhost:3001';
      
      const response = await axios.post(
        `${apiUrl}/api/admin/users/${userId}/reject`,
        { reason, note },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const requestChanges = async (userId, requirements) => {
    try {
      const token = localStorage.getItem('token');
      const apiUrl = process.env.REACT_APP_API_URL || 'http://localhost:3001';
      
      const response = await axios.post(
        `${apiUrl}/api/admin/users/${userId}/request-changes`,
        { requirements },
        { headers: { Authorization: `Bearer ${token}` } }
      );
