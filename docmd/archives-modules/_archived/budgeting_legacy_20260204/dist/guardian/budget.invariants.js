...
exports.BudgetInvariants = void 0;
class BudgetInvariants {
    /**
     * INV-BO-01: Unicité période
     * Un seul budget actif par période et par tenant
     */
    static invariantSingleActiveBudgetPerPeriod(newBudget, existingBudgets) {
        const overlapping = existingBudgets.filter((b) => b.tenantId === newBudget.tenantId &&
            b.status !== 'CLOSED' &&
            b.id !== newBudget.id &&
     ...
     */
    static invariantAllowedStateTransitions(currentStatus, newStatus) {
     ...
     */
    static invariantImmutableAfterValidation(budget) {
     ...
     */
    static invariantObjectivesQuantifiable(budget) {
        const violations = [];
     ...
     */
    static invariantCapacityNotExceeded(budget) {
        const violations = [];
     ...
     */
    static invariantObjectivesHaveCapacity(budget) {
     ...
     */
    static invariantObjectivesHaveCostStructure(budget) {
        if (budget.status === 'DRAFT') {
            return { ok: true, violations: [] };
        }
        const violations = [];
     ...
     */
    static invariantPaymentTermsDefined(budget) {
        if (budget.status === 'DRAFT') {
            return { ok: true, violations: [] };
        }
...
exports.BudgetInvariants = BudgetInvariants;
//# sourceMappingURL=budget.invariants.js.map