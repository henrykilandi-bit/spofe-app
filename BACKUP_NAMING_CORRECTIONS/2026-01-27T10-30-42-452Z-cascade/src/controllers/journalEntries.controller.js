...
import { JournalEntry, Company, sequelize } from '../models/index.js';
...
export const getAllEntries = async (req, res) => {
  try {
    ...
    console.log('getAllEntries - company_id:', company_id, 'req.query:', req.query);
...
export const updateEntry = async (req, res) => {
  ...
  try {
    ...
    const entry = await JournalEntry.findOne({ where: { id, company_id } });
    if (!entry) {
      await transaction.rollback();
      return error(res, 'Écriture non trouvée', 404);
    }
