...
import { ArchitectureViolation, AnalysisReport, FileAnalysisResult, Severity } from './types/architecture.types.js';
import { RuleContext } from './types/rule.interface.js';
...
export class AnalysisEngine {
  ...
  analyzeFile(filePath: string): FileAnalysisResult {
    try {
      ...
      if (fileName.endsWith('.process.ts')) context.fileType = 'process';
      else if (fileName.endsWith('.entity.ts')) context.fileType = 'entity';
      else if (fileName.endsWith('.service.ts')) context.fileType = 'service';
  ...
  private buildReport(
    ...
    for (const v of violations) {
      ...
      if (v.autoFixable) autoFixableCount++;
