...
class BackendBuilder {
  ...
  async scanSourceForBackendFiles() {
    console.log('\n🔍 SCAN PROFOND DES FICHIERS BACKEND...');
    
    try {
      // Patterns de fichiers backend à rechercher
      ...
      await this.scanDirectory(this.sourceRoot, '', backendPatterns, excludePatterns);
      
      this.log('Scan', 'SUCCESS', `Scan terminé`, `${this.backendFiles.length} fichiers backend identifiés`);
  ...
  async scanDirectory(dirPath, relativePath, patterns, excludes) {
    try {
      ...
      for (const item of items) {
        ...
        if (excludes.some(exclude => itemRelativePath.includes(exclude))) {
          continue;
        }
        ...
        if (stat.isDirectory()) {
          ...
          await this.scanDirectory(fullPath, itemRelativePath, patterns, excludes);
        } else if (stat.isFile()) {
          // Vérifier si le fichier correspond aux patterns
          if (this.matchesPatterns(item, patterns)) {
            // Classifier le fichier
            const classification = this.classifyFile(itemRelativePath);
  ...
  classifyFile(filePath) {
    ...
    if (lowerPath.includes('package.json') || lowerPath.includes('.env') || 
        lowerPath.includes('docker') || lowerPath.includes('vite.config')) {
      return 'CONFIG';
    }
    
    // Scripts de base de données
    if (lowerPath.includes('.sql') || lowerPath.includes('database') || 
        lowerPath.includes('mysql') || lowerPath.includes('db')) {
      return 'DATABASE';
    }
    
    // Scripts serveur/API
    if (lowerPath.includes('server') || lowerPath.includes('app') || 
    ...
    // Documentation
    if (lowerPath.includes('.md') || lowerPath.includes('readme') || 
        lowerPath.includes('doc')) {
      return 'DOCUMENTATION';
    }
    
    // Scripts de déploiement
    if (lowerPath.includes('.ps1') || lowerPath.includes('.sh') || 
...
`;
  ...
  async runFullBuild() {
    ...
    await this.scanSourceForBackendFiles();
