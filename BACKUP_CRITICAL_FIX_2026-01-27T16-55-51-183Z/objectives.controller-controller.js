...
import { strategicObjectiveDto, strategicObjectiveDtoArray, objectiveActionDto } from '../dto/index.js';
...
export const createObjective = async (req, res, next) => {
  try {
    ...
    logger.logInfo(`Objectif créé: ${objective.id} par utilisateur ${user_id}`);

    return success(
      res,
      {
        objectif: strategicObjectiveDto(objective),
...
export const listObjectives = async (req, res, next) => {
  try {
    ...
    logger.logInfo(`${objectives.count} objectifs listés pour utilisateur ${user_id}`);

    return success(res, {
      objectives: objectives.rows.map(o => strategicObjectiveDto(o)),
      pagination: {
...
export const getObjectiveDetail = async (req, res, next) => {
  try {
    ...
    return success(res, {
      objective: strategicObjectiveDto(objective),
      aiInsights: {
...
export const updateObjective = async (req, res, next) => {
  try {
    ...
    logger.logInfo(`Objectif ${id} mis à jour par utilisateur ${user_id}`);
...
export const deleteObjective = async (req, res, next) => {
  try {
    ...
    logger.logInfo(`Objectif ${id} supprimé (soft-delete) par utilisateur ${user_id}`);
...
export const restoreObjective = async (req, res, next) => {
  try {
    ...
    logger.logInfo(`Objectif ${id} restauré par utilisateur ${user_id}`);
...
export const createObjectiveAction = async (req, res, next) => {
  try {
    ...
    return success(res, { action: objectiveActionDto(action) }, 201, 'Action créée');
  } catch (err) {
...
export const batchUpdateProgression = async (req, res, next) => {
  try {
    ...
    logger.logInfo(`${results.filter(r => r.success).length} objectifs mis à jour en batch par ${user_id}`);
