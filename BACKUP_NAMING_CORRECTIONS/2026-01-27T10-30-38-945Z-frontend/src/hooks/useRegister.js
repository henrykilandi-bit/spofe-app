...
export const useRegister = () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/api/auth/register`,
        {
          ...
          groupeId: formData.groupeId || null,
  ...
  const validateInvitation = useCallback(async (token, email) => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/api/auth/validate-invitation`,
        {
          params: { token, email },
          timeout: 5000
        }
   ...
   */
  const fetchGroupeInfo = useCallback(async (groupeId) => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/api/groupes/${groupeId}`,
        { timeout: 5000 }
      );
