  ...
  try {
    ...
    if (missingFields.length > 0) {
        ...
        try {
          ...
          if (rowCount[0].count > 0) {
            ...
            await connection.execute(`
              ALTER TABLE consulting_firms 
              ADD COLUMN name VARCHAR(255) NOT NULL DEFAULT 'Cabinet Sans Nom' AFTER id
            `);
          } else {
      ...
      if (!currentFields.includes('registration_number')) {
        ...
        try {
          await connection.execute(`
            ALTER TABLE consulting_firms 
            ADD COLUMN registration_number VARCHAR(255) NULL COMMENT 'Numéro d\\'enregistrement (RCCM/SIRET)'
          `);
      ...
      if (!currentFields.includes('deleted_at')) {
        ...
        try {
          await connection.execute(`
            ALTER TABLE consulting_firms 
            ADD COLUMN deleted_at TIMESTAMP NULL COMMENT 'Date de suppression (soft delete)'
          `);
    ...
    try {
      ...
      const testInsert = `
        ...
        VALUES ('Cabinet Test Vérification', 'independent', 'Cabinet de test pour vérification finale', NOW(), NOW())
      `;
      ...
      // Test de mise à jour
      await connection.execute(`
        UPDATE consulting_firms 
        SET registration_number = 'TEST-123456789' 
        WHERE id = ?
      `, [newFirm[0].id]);
      ...
      // Nettoyer le test
      await connection.execute('DELETE FROM consulting_firms WHERE name = "Cabinet Test Vérification"');
