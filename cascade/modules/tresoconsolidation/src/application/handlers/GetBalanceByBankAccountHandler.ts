/**
 * GetBalanceByBankAccountHandler.ts
...
import { GetBalanceByBankAccountQuery } from '../queries/GetBalanceByBankAccountQuery';

export class GetBalanceByBankAccountHandler {
  constructor(private readonly readPort: TresoconsolidationReadPort) {}

  async execute(query: GetBalanceByBankAccountQuery): Promise<any[]> {
    // Guardian P0 — Read-only strict
    TresoconsolidationGuardian.assertReadOnly(true, 'GET');

    return this.readPort.getBalanceByBankAccount(
      query.tenantId,
      query.bankAccountId,
      query.asOf
    );
